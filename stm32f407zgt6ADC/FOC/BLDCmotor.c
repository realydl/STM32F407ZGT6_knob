...
//		mid_angle = getAngle();//AS5600
		mid_angle = getAngle_as5047p();//AS5047P
...
//			setPhaseVoltage(voltage_sensor_align, 0,  angle);
		...
		my_delay_ms(1000);
//		end_angle = getAngle();//AS5600
		end_angle = getAngle_as5047p();
...
//			sensor_direction=CCW;
		...
		if( fabs(moved*pole_pairs2 - _2PI) > 0.5f )  // 0.5 is arbitrary number it can be lower or higher!
		{
			printf("fail - estimated pp:");
			pole_pairs2 = (int)(_2PI/moved + 0.5f);     //浮点数转整形，四舍五入
			printf("%d\r\n",pole_pairs2);
		}
		else printf("OK!\r\n");
	}
...
//		sensor_offset = shaftAngle();// shaft angle
		...
		my_delay_ms(700);
		zero_electric_angle2 = _normalizeAngle(_electricalAngle(sensor_direction2*getAngle2(), pole_pairs2));
		zero_angle2 = _normalizeAngle(sensor_direction2*getAngle2());
		my_delay_ms(20);
...
////多段旋钮的目标设计
////和手感有关可以设置成线性的也可以设置成正弦函数型等各种波形
////四段式旋钮设置，点位分别在Encode的0,90,180,270
//float Multistage_knob_move(float now_position){
//	char num = (uint32_t)(now_position/45) + 1;
//	switch(num){
//		case 1://向0运动
//			current_sp = arm_sin_f32(-(now_position - 0));//+
//			break;
//		case 2://向90运动
//			current_sp = arm_sin_f32(now_position - 90.0);//-
//			break;
//		case 3://向90运动
//			current_sp = arm_sin_f32(now_position - 90.0);//+
//			break;
//		case 4://向180运动
//			current_sp = arm_sin_f32(now_position - 180.0);//-
//			break;
//		case 5://向180运动
//			current_sp = arm_sin_f32(now_position - 180.0);//+
//			break;
//		case 6://向270运动
//			current_sp = arm_sin_f32(now_position - 270.0);//-
//			break;
//		case 7://向270运动
//			current_sp = arm_sin_f32(now_position - 270.0);//+
//			break;
//		case 8://向0运动
//			current_sp = arm_sin_f32(-(now_position - 360.0));//-
//		break;
//		default:
//			current_sp = 0;
//	}
...
//	current_sp = 2 *(shaft_angle2 - shaft_angle);
	current_sp = 2*(ABS_Angle2() - ABS_Angle());
}

void move_torque2(void)
{
//	current_sp2 = 2 *(shaft_angle - shaft_angle2);
	current_sp2 = 6*(ABS_Angle() - ABS_Angle2());
}


void knob_move(void){
	current_sp = ABS_Angle();
}
...
float move_velocity2(float velocity_target2){
	shaft_velocity2 = shaftVelocity2();//测速
	// velocity set point
	shaft_velocity_sp2 = velocity_target2;
	// calculate the torque command
	current_sp2 = PIDoperator(&PID_velocity2,(shaft_velocity_sp2 - shaft_velocity2)); // if current/foc_current torque control
//	current_sp = PIDoperator(&PID_velocity,(shaft_velocity - shaft_velocity_sp)); // if current/foc_current torque control
	// if torque controlled through voltage control 
//	if(torque_controller == Type_voltage)
//	{
...
//AS5047P
void Knob_loop(void){
	shaft_angle = shaftAngle_AS5047P();// shaft angle
	electrical_angle = electricalAngle();// electrical angle - need shaftAngle to be called first
